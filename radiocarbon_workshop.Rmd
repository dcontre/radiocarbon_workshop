...
Now we might suggest that cemetery use began around `r min(bur20_low$ages)` and ended by about `r max(bur20_low$ages)`. We might even think about suggesting - wrongly, it turns out - that use increased over time. And what might our interpretion be if we had only those original five individuals to work with?
...
To generate a basis for discussion for our second session, please explore this set (Table 6.1, and available in 'BurialSim2_sim.csv') of radiocarbon ages on 20 samples from 10 burials (thinking about the same cemetery that we explored above). Estimate the span of use of the cemetery. Assume a terrestrial diet for all individuals. Drop dates that you don't trust - but justify excluding them. 
