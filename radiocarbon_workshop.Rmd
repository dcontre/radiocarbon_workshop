...
```{r echo = F, out.width='100%', out.extra='style="padding:10px"', fig.cap="Entierro 11/92, Huaca Cao Viejo (Complejo Arqueológico El Brujo archives)."}
...
```{r echo = F, out.width='100%', out.extra='style="padding:10px"', fig.cap="The carbon cycle."}
...
As noted above, calibration is necessary because atmospheric radiocarbon has varied over time. It also varies in space (Fig. 5.11), driven at global scales by the contrast in relative land area in the northern and southern hemispheres and the relative segregation of the air masses in those hemispheres [see @marsh2018, 926-927 for an overview]. Two distinct calibration curves are the result: IntCal for the northern hemisphere and SHCal for the southern; current versions, published in 2020, are IntCal20 [@reimer2020] and SHCal20 [@hogg2020]. These are only modestly different (Figure 5.12), but that slight difference can affect where the radiocarbon determination intersects the calibration curve (see Fig. 5.13, where the bulk of the probability shifts only slightly, but the tail moves from pre-900 CE to post-1050 CE).

```{r echo = F, out.width='100%', out.extra='style="padding:10px"', fig.cap="Hua and colleagues (2022) illustrate 20th-21st century  variability in atmospheric radiocarbon, based on atmospheric CO2 sampling (triangles) and Δ14C tree-ring records (circles)."}
knitr::include_graphics("imgs/Hua2022_Fig1.png")
```

```{r echo = F, fig.cap = "IntCal20 and SHCal20 from 1000 BCE to 1000 CE. Plotted using the R package **rice** (Blaauw 2024)."}
draw.ccurve(-1000, 1000, BCAD=TRUE, cc2='shcal20', add.yaxis=TRUE)
```

```{r echo = F, out.width='50%', fig.cap = "Calibrated age of LabID-XXX (1100 ±40). Calibrated using the R package **rice** (Blaauw 2024).", fig.show = "hold"}
rice::calibrate(1100, 40, BCAD = T)
...
You can select calibration curves in OxCal by clicking on 'Options' and then 'Calibration' (Fig. 5.14).

```{r echo = F, out.width='60%', out.extra='style="float:right; padding:10px"', fig.cap="Calibration curve selection in OxCal."}
...
As Figure 5.11 implies, it's not always obvious which calibration curve is appropriate, particularly at tropical latitudes. Atmospheric circulation patterns also vary seasonally: note that Fig. 5.11 illustrates two different - December–February [DJF] and June–August [JJA] locations for the tropical low pressure band [TLPB], which Hua and colleagues prefer to the inter-tropical convergence zone [ITCZ] because it can be more precisely described in terrestrial areas. This is of particular importance for South American archaeology (Fig. 5.19); note how far south the TLPB moves during the austral summer. Atmospheric circulation patterns have also varied over time, since the location of the ITCZ is closely linked to climate changes [see, *e.g.*, @mcgee2014].

```{r echo = F, out.width='100%', out.extra='style="padding:10px"', fig.cap="Proposed calibration curves in South America. Note that these are not intended to be spatially precise."}
...
```{r echo = F, out.width='60%', out.extra='style="padding:10px"', fig.cap="Importing multiple dates."}
...
```{r echo = F, out.width='100%', out.extra='style="padding:10px"', fig.cap="Plotting a phase model."}
...
```{r echo = F, out.width='100%', out.extra='style="padding:10px"', fig.cap="KDE and boundaries."}
...
-   We don't know anything about this individual's diet, but either consider a marine dietary component *unlikely* or simply don't consider the issue at all, and calibrate the radiocarbon result using only the atmospheric curve (SHCal20, in this case), as we would for any other terrestrial sample. (Figure 5.34, 01a)
-   We don't know anything about this individual's diet, but consider it *likely* that they had a significant marine dietary component, and realize that our calibration should account for this. We know the local $\Delta{R}$ is -200 ±75 years, and can thus model the effects of a diet that might range anywhere from 0 - 100% marine. (Figure 5.34, 01b)
-   Based on the C/N isotopic composition of the sampled bone and the application of a mixing model, we are able to estimate that their diet was 50 ±20% marine, and we know the local $\Delta{R}$ is -200 ±75 years. (Figure 5.34, 01c)

```{r echo = F, out.width='100%', out.extra='style="padding:10px"', fig.cap="Effects of different assumptions about a 50% marine diet on a calibrated age estimate."}
...
With results that look like those in Figure 5.36 ('View' --\> 'Plot dates')... <!-- problem of set.seed() not working here; results discussed below will change -->

```{r echo = F, fig.cap = "Bounded phase model for $^{14}C$ dates from Burial 11/92."}
